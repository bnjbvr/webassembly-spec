(function nop_wast_js() {
$$ = instance("");

assert_return(() => call($$, "as-func-first", []), 1);

assert_return(() => call($$, "as-func-mid", []), 2);

assert_return(() => call($$, "as-func-last", []), 3);

assert_return(() => call($$, "as-func-everywhere", []), 4);

assert_return(() => call($$, "as-drop-last", [0]));

assert_return(() => call($$, "as-drop-everywhere", [0]));

assert_return(() => call($$, "as-select-mid1", [3]), 3);

assert_return(() => call($$, "as-select-mid2", [3]), 3);

assert_return(() => call($$, "as-select-last", [3]), 3);

assert_return(() => call($$, "as-select-everywhere", [3]), 3);

assert_return(() => call($$, "as-block-first", []), 2);

assert_return(() => call($$, "as-block-mid", []), 2);

assert_return(() => call($$, "as-block-last", []), 3);

assert_return(() => call($$, "as-block-everywhere", []), 4);

assert_return(() => call($$, "as-loop-first", []), 2);

assert_return(() => call($$, "as-loop-mid", []), 2);

assert_return(() => call($$, "as-loop-last", []), 3);

assert_return(() => call($$, "as-loop-everywhere", []), 4);

assert_return(() => call($$, "as-if-condition", [0]));

assert_return(() => call($$, "as-if-condition", [-1]));

assert_return(() => call($$, "as-if-then", [0]));

assert_return(() => call($$, "as-if-then", [4]));

assert_return(() => call($$, "as-if-else", [0]));

assert_return(() => call($$, "as-if-else", [3]));

assert_return(() => call($$, "as-br-last", [6]), 6);

assert_return(() => call($$, "as-br-everywhere", [7]), 7);

assert_return(() => call($$, "as-br_if-mid", [5]), 5);

assert_return(() => call($$, "as-br_if-last", [6]), 6);

assert_return(() => call($$, "as-br_if-everywhere", [7]), 7);

assert_return(() => call($$, "as-br_table-mid", [5]), 5);

assert_return(() => call($$, "as-br_table-last", [6]), 6);

assert_return(() => call($$, "as-br_table-everywhere", [7]), 7);

assert_return(() => call($$, "as-return-last", [6]), 6);

assert_return(() => call($$, "as-return-everywhere", [7]), 7);

assert_return(() => call($$, "as-call-mid1", [3, 1, 2]), 2);

assert_return(() => call($$, "as-call-mid2", [0, 3, 1]), 2);

assert_return(() => call($$, "as-call-last", [10, 9, -1]), 20);

assert_return(() => call($$, "as-call-everywhere", [2, 1, 5]), -2);

assert_return(() => call($$, "as-unary-last", [30]), 1);

assert_return(() => call($$, "as-unary-everywhere", [12]), 2);

assert_return(() => call($$, "as-binary-mid", [3]), 6);

assert_return(() => call($$, "as-binary-last", [3]), 6);

assert_return(() => call($$, "as-binary-everywhere", [3]), 6);

assert_return(() => call($$, "as-test-last", [0]), 1);

assert_return(() => call($$, "as-test-everywhere", [0]), 1);

assert_return(() => call($$, "as-compare-mid", [3]), 0);

assert_return(() => call($$, "as-compare-last", [3]), 0);

assert_return(() => call($$, "as-compare-everywhere", [3]), 1);

assert_return(() => call($$, "as-grow_memory-last", [2]), 1);

assert_return(() => call($$, "as-grow_memory-everywhere", [12]), 3);

assert_invalid("\x00\x61\x73\x6d\x0d\x00\x00\x00\x01\x85\x80\x80\x80\x00\x01\x60\x00\x01\x7f\x03\x82\x80\x80\x80\x00\x01\x00\x0a\x89\x80\x80\x80\x00\x01\x83\x80\x80\x80\x00\x00\x01\x0b");

assert_invalid("\x00\x61\x73\x6d\x0d\x00\x00\x00\x01\x85\x80\x80\x80\x00\x01\x60\x00\x01\x7e\x03\x82\x80\x80\x80\x00\x01\x00\x0a\x89\x80\x80\x80\x00\x01\x83\x80\x80\x80\x00\x00\x01\x0b");

assert_invalid("\x00\x61\x73\x6d\x0d\x00\x00\x00\x01\x85\x80\x80\x80\x00\x01\x60\x00\x01\x7d\x03\x82\x80\x80\x80\x00\x01\x00\x0a\x89\x80\x80\x80\x00\x01\x83\x80\x80\x80\x00\x00\x01\x0b");

assert_invalid("\x00\x61\x73\x6d\x0d\x00\x00\x00\x01\x85\x80\x80\x80\x00\x01\x60\x00\x01\x7c\x03\x82\x80\x80\x80\x00\x01\x00\x0a\x89\x80\x80\x80\x00\x01\x83\x80\x80\x80\x00\x00\x01\x0b");

})();