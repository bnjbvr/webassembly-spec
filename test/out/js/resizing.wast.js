(function resizing_wast_js() {
$$ = instance("");

assert_return(() => call($$, "size", []), 0);

assert_trap(() => call($$, "store_at_zero", []));

assert_trap(() => call($$, "load_at_zero", []));

assert_trap(() => call($$, "store_at_page_size", []));

assert_trap(() => call($$, "load_at_page_size", []));

assert_return(() => call($$, "grow", [1]), 0);

assert_return(() => call($$, "size", []), 1);

assert_return(() => call($$, "load_at_zero", []), 0);

assert_return(() => call($$, "store_at_zero", []));

assert_return(() => call($$, "load_at_zero", []), 2);

assert_trap(() => call($$, "store_at_page_size", []));

assert_trap(() => call($$, "load_at_page_size", []));

assert_return(() => call($$, "grow", [4]), 1);

assert_return(() => call($$, "size", []), 5);

assert_return(() => call($$, "load_at_zero", []), 2);

assert_return(() => call($$, "store_at_zero", []));

assert_return(() => call($$, "load_at_zero", []), 2);

assert_return(() => call($$, "load_at_page_size", []), 0);

assert_return(() => call($$, "store_at_page_size", []));

assert_return(() => call($$, "load_at_page_size", []), 3);

$$ = instance("");

assert_return(() => call($$, "grow", [0]), 0);

assert_return(() => call($$, "grow", [1]), 0);

assert_return(() => call($$, "grow", [0]), 1);

assert_return(() => call($$, "grow", [2]), 1);

assert_return(() => call($$, "grow", [800]), 3);

$$ = instance("");

assert_return(() => call($$, "grow", [0]), 0);

assert_return(() => call($$, "grow", [1]), 0);

assert_return(() => call($$, "grow", [1]), 1);

assert_return(() => call($$, "grow", [2]), 2);

assert_return(() => call($$, "grow", [6]), 4);

assert_return(() => call($$, "grow", [0]), 10);

assert_return(() => call($$, "grow", [1]), -1);

})();