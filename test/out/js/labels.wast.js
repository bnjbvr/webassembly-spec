(function labels_wast_js() {
$$ = instance("");

assert_return(() => call($$, "block", []), 1);

assert_return(() => call($$, "loop1", []), 5);

assert_return(() => call($$, "loop2", []), 8);

assert_return(() => call($$, "loop3", []), 1);

assert_return(() => call($$, "loop4", [8]), 16);

assert_return(() => call($$, "loop5", []), 2);

assert_return(() => call($$, "if", []), 5);

assert_return(() => call($$, "if2", []), 5);

assert_return(() => call($$, "switch", [0]), 50);

assert_return(() => call($$, "switch", [1]), 20);

assert_return(() => call($$, "switch", [2]), 20);

assert_return(() => call($$, "switch", [3]), 3);

assert_return(() => call($$, "switch", [4]), 50);

assert_return(() => call($$, "switch", [5]), 50);

assert_return(() => call($$, "return", [0]), 0);

assert_return(() => call($$, "return", [1]), 2);

assert_return(() => call($$, "return", [2]), 2);

assert_return(() => call($$, "br_if0", []), 29);

assert_return(() => call($$, "br_if1", []), 1);

assert_return(() => call($$, "br_if2", []), 1);

assert_return(() => call($$, "br_if3", []), 2);

assert_return(() => call($$, "br", []), 1);

assert_return(() => call($$, "shadowing", []), 1);

assert_return(() => call($$, "redefinition", []), 5);

assert_invalid("\x00\x61\x73\x6d\x0d\x00\x00\x00\x01\x84\x80\x80\x80\x00\x01\x60\x00\x00\x03\x82\x80\x80\x80\x00\x01\x00\x0a\x91\x80\x80\x80\x00\x01\x8b\x80\x80\x80\x00\x00\x02\x40\x41\x01\x0d\x00\x8c\x01\x0b\x0b");

assert_invalid("\x00\x61\x73\x6d\x0d\x00\x00\x00\x01\x84\x80\x80\x80\x00\x01\x60\x00\x00\x03\x82\x80\x80\x80\x00\x01\x00\x0a\x94\x80\x80\x80\x00\x01\x8e\x80\x80\x80\x00\x00\x02\x40\x43\x00\x00\x00\x00\x41\x01\x0d\x00\x0b\x0b");

assert_invalid("\x00\x61\x73\x6d\x0d\x00\x00\x00\x01\x84\x80\x80\x80\x00\x01\x60\x00\x00\x03\x82\x80\x80\x80\x00\x01\x00\x0a\x94\x80\x80\x80\x00\x01\x8e\x80\x80\x80\x00\x00\x02\x40\x43\x00\x00\x00\x00\x41\x01\x0d\x00\x0b\x0b");

})();